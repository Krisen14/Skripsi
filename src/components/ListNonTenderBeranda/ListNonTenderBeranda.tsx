...
import './ListNonTenderBeranda.css';
...
export default function TenderGrouped() {
  ...
  useEffect(() => {
    const fetchTenders = async () => {
      try {
        const res = await fetch('/api/nontender'); // Ganti jika endpoint-nya beda
  ...
  useEffect(() => {
    const groupByCategory = () => {
      ...
      const filtered = tenders.filter(t => t.status === 'Selesai');
  ...
  return (
    <div className="list-container">
      <div className="header-tender-beranda">
        <h2>Non Tender <Link href="/caripaket">Lihat Semua »</Link></h2>
      </div>
          ...
          <Slider ref={(el) => (sliderRefs.current[index] = el)} {...sliderSettings}>
            {list.slice(0, 10).map((tender) => (
              <div className="tender-card" key={tender._id}>
                <div className='namatender'><strong>Nama:</strong> {tender.namaTender}</div>
                <div className="card-top">
                  ...
                  <div><strong>Rp {tender.nilaiHps.toLocaleString('id-ID')}</strong></div>
      ...
      <Modal isOpen={!!selectedTender} onClose={() => setSelectedTender(null)}>
        ...
        <div className="modal-actions">
          <Link href={`/nontender/${selectedTender?._id}`} className="btn-detail">Detail Selengkapnya</Link>
