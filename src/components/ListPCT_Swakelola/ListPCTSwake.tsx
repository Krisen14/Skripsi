...
export default function ListPCTSwake({ filters }: ListProps) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await fetch('/api/pct_swakelola');
  ...
  return (
    ...
    <h1 className="list-title">Daftar Pencatatan Swakelola</h1>
            ...
            {currentUsers.length === 0 ? (
              ...
              currentUsers.map((user) => (
                <tr key={user._id} className="tender-row">
                  ...
                  <td>
                    <Link href={`/pct_swakelola/${user._id}`} className="tender-link">
                      {user.namaTender}
                    </Link><br />
